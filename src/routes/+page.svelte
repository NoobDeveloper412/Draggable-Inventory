<script lang="ts">
    ...
    // Second grid to receive items from the first grid
    ...
    function handleDrop(event: DragEvent, target: 'items' | 'droppedItems') {
        ...
        if (itemIndex !== null && origin !== null) {
            ...
            if (origin === 'items' && target === 'droppedItems') {
                // Move item from the main grid to the second grid
                ...
                // Move item back to the main grid from the second grid
...
<div class=" h-full px-20"> 
    {#if $isVisible}
        <div class="flex items-center justify-bewteen h-screen  p-6 rounded-lg flex gap-8">
            <!-- First Grid (Main Items) -->
            <div class="w-1/2 bg-[#596475] border border-2 rounded-sm">
                ...
                <div class="grid grid-cols-3 gap-4  p-4 rounded-lg" on:dragover={handleDragOver} on:drop={(e) => handleDrop(e, 'items')}>
                    {#each $items as item, itemIndex (itemIndex)}
                        <button
                            class="grid-item p-4 bg-[#596475] border cursor-pointer border-white rounded-lg shadow-md hover:bg-gray-200 transition-all cursor-pointer"
                            draggable="true"
            ...
            <div class="w-1/2 bg-[#596475] border border-2 rounded-sm">
                ...
                <div class="grid grid-cols-3 gap-4 p-4 rounded-lg" on:dragover={handleDragOver} on:drop={(e) => handleDrop(e, 'droppedItems')}>
                    {#each $droppedItems as item, itemIndex (itemIndex)}
                        <button
                            class="grid-item p-4 bg-gray-100 border border-whites rounded-lg shadow-md hover:bg-gray-200 transition-all cursor-pointer"
                            draggable="true"
